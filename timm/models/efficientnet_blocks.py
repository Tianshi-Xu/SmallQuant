...
class DepthwiseSeparableConv(nn.Module):
    ...
    def __init__(
            ...
            se_layer=None, drop_path_rate=0., has_dw_skip=False,
            dw_chl_per_group=1):
        ...
        self.has_dw_skip = has_dw_skip and (stride == 1)
        self.dw_chl_per_group = dw_chl_per_group

        self.conv_dw = create_conv2d(
            in_chs, in_chs, dw_kernel_size, stride=stride, dilation=dilation,
            padding=pad_type, groups=groups // dw_chl_per_group)
        self.bn1 = norm_act_layer(in_chs, inplace=True, apply_act=False)
        self.relu1 = act_layer(inplace=True)
        ...
        # self.bn2 = norm_act_layer(out_chs, inplace=True, apply_act=False)
        self.drop_path = DropPath(drop_path_rate) if drop_path_rate else nn.Identity()
...
class InvertedResidual(nn.Module):
    ...
    def __init__(
            ...
            norm_layer=nn.BatchNorm2d, se_layer=None, conv_kwargs=None,
            drop_path_rate=0., has_dw_skip=False, dw_chl_per_group=1):
        ...
        self.conv_dw = create_conv2d(
            ...
            groups=groups // dw_chl_per_group, padding=pad_type, **conv_kwargs)
        # self.bn2 = norm_act_layer(mid_chs, inplace=True)
        self.bn2 = norm_act_layer(mid_chs, apply_act=False)
        self.relu2 = act_layer(inplace=True)
