...
def _decode_block_str(block_str):
    ...
    for op in ops:
        ...
        if op == 'noskip':
            ...
            if v == 're':
                ...
                continue
            options[key] = value
        elif op.startswith('dwg'):
            v = int(op[3:])
            options['dw_chl_per_group'] = v
        else:
            # all numeric options
    ...
    if block_type == 'ir':
        block_args.update(dict(
            ...
            has_dw_skip=dw_skip is True,
            dw_chl_per_group=options.get('dw_chl_per_group', 1),
        ))
        if 'cc' in options:
...
class EfficientNetBuilder:
    ...
    def __init__(self, output_stride=32, pad_type='', round_chs_fn=round_channels, se_from_exp=False,
                 act_layer=None, norm_layer=None, se_layer=None,
                 drop_path_rate=0., feature_location=''):
