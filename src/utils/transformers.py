...
class Attention(Module):
    ...
    def __init__(
        ...
        use_skip=False, use_softmax=True,
    ):
        super().__init__()
        self.dim = dim
        ...
        if self.use_softmax:
            ...
            self.attn_func = ReLUSoftmax()

    def forward(self, x):
        B, N, C = x.shape
        # qkv dimension: 3, B, num_heads, N, C // num_heads
...
class TransformerEncoderLayer(Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 ...
                 use_layer_scale=False, use_skip=False, use_relu=False,
                 use_softmax=True):
        ...
        # self.activation = F.gelu if not use_relu else F.relu
        self.activation = nn.GELU() if not use_relu else nn.ReLU(inplace=True)

        init_values = 1e-4
        self.use_layer_scale = use_layer_scale

        if self.use_layer_scale:
            self.gamma_1 = torch.nn.Parameter(init_values * torch.ones(d_model), requires_grad=True)
            self.gamma_2 = torch.nn.Parameter(init_values * torch.ones(d_model), requires_grad=True)

    def forward(self, src: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        src2 = self.self_attn(self.pre_norm(src))
        src2 = self.gamma_1 * src2 if self.use_layer_scale else src2
        src = src + self.drop_path(src2)
        src = self.norm1(src)
        src2 = self.linear2(self.dropout1(self.activation(self.linear1(src))))
        src2 = self.gamma_2 * src2 if self.use_layer_scale else src2
    ...
    def __init__(self,
        ...
        self.blocks = ModuleList([
            TransformerEncoderLayer(d_model=embedding_dim, nhead=num_heads,
                                    ...
                                    attention_dropout=attention_dropout,
                                    drop_path_rate=dpr[i], use_layer_scale=use_layer_scale,
                                    use_skip=use_skip, use_relu=use_relu,
                                    use_softmax=use_softmax)
