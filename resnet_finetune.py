...
parser.add_argument('--percentage', type=float, default=0,
                    help='description of the mix precision')
...
def get_qat_model(model, args):
        ...
        for name, module in model.named_modules():
            if type(module)==nn.Conv2d and name in args.qmodules:
                ...
                acfg = {
                    ...
                    "symmetric": False,  # not args.aq_asym,
                    "per_channel": False,
                    "normalize_first": False,
                    "p2_round_scale": True,  # scaling factor is power-of-2?
                }
    ...
    for m in args.resq_modules:
        acfg = {
            ...
            "all_positive": False,
            "symmetric": False,  # not args.resq_asym,
            "per_channel": False,
            "normalize_first": False,
            "p2_round_scale": True,  # scaling factor is power-of-2?
        }
...
if __name__ == '__main__':
    ...
    for i in [3,6,8,16]:
        args.resq_bitw = i
        args.output='./output/eval/resnet32/'
        args.exp_name = 'w4a3_resq'+str(i)
        main((args, args_text))
    exit()

    file = open("resnet32_comm.txt")
    comm_dict = {}
    ...
    previous_k = 0.4
    for k in sorted(comm_dict,reverse=True):
        v = comm_dict[k]
        args.output='./output/eval/resnet32/'
        args.exp_name = 'comm'+str(k)+'resq4'
        args.bit_setting = v
        #if not previous_k == 0 and k <= 0.4:
        #    args.initial_checkpoint = args.output+"comm"+str(previous_k)+"/model_best.pth.tar"
        previous_k = k
        main((args, args_text))
        continue
    exit()
    ...
    for k,v in comm_dict.items():
        args.percentage = k
        args.output='./output/eval/resnet32/'+str(args.percentage)
        bit_setting = v
        cur_setting = [4 for i in range(len(bit_setting))]

        index = len(bit_setting)-1
        change_bound = 100
        stage = 0

        while index >= 0:
            change_num = 0 
            change_flag = False # in case of the last change
            while change_num <= change_bound and index >= 0:
                if not cur_setting[index] == bit_setting[index]:
                    change_flag = True
                    print(f"change index{index} from {cur_setting[index]} to {bit_setting[index]}")
                    change_num += (cur_setting[index] - bit_setting[index])
                    cur_setting[index] = bit_setting[index]
                index -= 1
            if change_flag:
                args.bit_setting = cur_setting
                args.exp_name = f'{stage}'
                if not stage==0:
                    args.initial_checkpoint = args.output+"/"+str(stage-1)+"/model_best.pth.tar"
                if not os.path.exists(args.output+"/"+str(stage)+"/model_best.pth.tar"):
                    main((args, args_text))
                stage += 1
        ...
        for stage in range(total_stage):
            args.stage=stage
            bits = []
            for j in range(len(bit_setting)):
                args.stage=stage
                if j < (total_stage-stage-1)/total_stage*len(bit_setting):
                    bits.append(4)
                else:
                    bits.append(bit_setting[j])
            args.bit_setting = bits
            if not stage==0:
                args.initial_checkpoint = args.output+"/stage"+str(previous_stage)+"/model_best.pth.tar"
            main((args, args_text))
            previous_stage = stage #to handle the unnecessary stage """
