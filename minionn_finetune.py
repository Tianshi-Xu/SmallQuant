...
if __name__ == '__main__':
    ...
    args.output='./output/eval/minionn_nokd/'
    ...
    for k in sorted(comm_dict,reverse=True):
        ...
        args.bit_setting = v
        args.resume = args.output+"comm"+str(previous_k)+"/model_best.pth.tar"
        print(args.resume)
        previous_k = k
        main((args, args_text))
        continue
        ...
        if v[0] > 4:
            args.description = 'comm'+str(k)
            args.bit_setting = v
            args.resume = 'minionn_a4w8.pth.tar'
            main((args, args_text))
        else:
            stage_0 = 'minionn_'+str(v[0])
            resume = stage_0
            if not os.path.exists('output/eval/'+stage_0+'-minionn-32/model_best.pth.tar'):
                args.description = stage_0
                args.bit_setting = [v[0],8,8,8,8,8]
                args.resume='minionn_a4w8.pth.tar'
                main((args, args_text))

            if v[0] + v[1] <= 6:
                stage_1 = 'minionn_'+str(v[0])+str(v[1])
                resume = stage_1
                if not os.path.exists('output/eval/'+stage_1+'-minionn-32/model_best.pth.tar'):
                    args.description = stage_1
                    args.bit_setting = [v[0],v[1],8,8,8,8]
                    args.resume='output/eval/'+stage_0+'-minionn-32/model_best.pth.tar'
                    main((args, args_text))

            args.description = 'comm'+str(k)
            args.bit_setting = v
            args.resume = 'output/eval/'+resume+'-minionn-32/model_best.pth.tar'
            main((args, args_text))
        exit()
        stage_0 = 'minionn_'+str(v[0])+str(v[1])
        if not os.path.exists('output/eval/minionn/'+stage_0+'-minionn-32/model_best.pth.tar'):
            args.description = stage_0
            args.bit_setting = [v[0],v[1],8,8,8,8]
            if v[0]==3:
                args.resume = 'output/eval/minionn/'+'minionn_3'+'-minionn-32/model_best.pth.tar'
            else:
                args.resume='minionn_a4w8.pth.tar'
            main((args, args_text))
        args.description = 'comm'+str(k)
        args.bit_setting = v
        args.resume = 'output/eval/minionn/'+stage_0+'-minionn-32/model_best.pth.tar'
        main((args, args_text))
